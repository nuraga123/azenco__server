...
describe('Shopping Cart Controller', () => {
  ...
  it('should get all cart items', async () => {
    ...
    expect(response.body.find((item) => item.partId === myPartId)).toEqual(
  ...
  it('should delete  one partId method cart item', async () => {
    const login = await request(app.getHttpServer())
      .post('/users/login')
      .send({ username: mockedUser.username, password: mockedUser.password });
    ...
    expect(response.body.find((el) => el.partId === 1)).toBeUndefined();
    expect(response.body).toStrictEqual([]);
  });

  it('should delete  all userId method cart item', async () => {
    const login = await request(app.getHttpServer())
      .post('/users/login')
      .send({ username: mockedUser.username, password: mockedUser.password });
